...
- [x] **대표·데모 계정 비밀번호 재설정** (2026-08-23 완료) — 노출됐던 해시 무효화.
      대표 계정=대표 직접 변경, 데모(`demo-screenshots@ori.pics`)=DB 해시 회전(bcrypt cost10,
      새 비번은 `.secrets/demo-screenshots-account.txt`). 외부 접근 정황 없던 예방 조치.
