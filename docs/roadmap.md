...
**라이브러리**: `@contentauth/c2pa-node` (스코프드 패키지, v0.5.5 기준 — 2026-05-07 검증). 리포지토리 이름은 `c2pa-node-v2`이지만 npm 패키지명은 다름. ESM 전용, Node 18.20.2+ 요구. 구 unscoped `c2pa-node`는 deprecated.

**SDK 통합 실패 시 폴백** (예: Issue #58 glibc 호환성 문제):
- `c2patool` CLI를 `child_process.execFile`로 호출
- Vercel Sandbox 또는 외부 워커(Fly.io 등)에서 서명 처리
- C2PA Cloud Signing 서비스 호출

**Vercel 호환성 위험 사전 검증 필수**: 본 통합 코드 작성 전에 Preview 환경에 import 스텁만 배포해 패키지 로드 성공 여부 확인.
...
### A.5 코드 골격 — `apps/web/src/lib/oripics-stamp/c2pa.ts`

(모노레포 Phase 4 후엔 `packages/stamp/src/c2pa.ts`. 현재는 apps/web 내부.)

```ts
// ⚠️ v2 API 검증됨 (2026-05-07). builder.sign() 반환 시맨틱은 런타임 추가 검증 필요.
...
export async function attachC2paManifest(input: C2paAttachInput): Promise<{ buffer: Buffer }> {
  ...
  // 첫 action은 c2pa.created 또는 c2pa.opened 필수 (C2PA 2.2 spec)
  const actions: any[] = [
    {
      action: 'c2pa.created',
      when: input.timestamp,
      digitalSourceType: input.tier === 'verified'
  ...
  const manifestSpec = {
    claim_generator: input.tier === 'verified' ? 'oripics/0.1.0 (mobile-native)' : 'oripics/0.1.0',
  ...
  // v2 API: LocalSigner.newSigner(certBuf, keyBuf, alg, tsaUrl?)
  const signer = LocalSigner.newSigner(
    Buffer.from(CERT_PEM),
    Buffer.from(KEY_PEM),
    'es256',
    // 'https://timestamp.digicert.com'  // 정식 인증서 도입 후 활성화
  );

  // v2 API: Builder.withJson(jsonString)
  const builder = Builder.withJson(JSON.stringify(manifestSpec));

  const inputAsset  = { buffer: input.pngBuffer,  mimeType: 'image/png' };
  const outputAsset = { buffer: Buffer.alloc(0),  mimeType: 'image/png' };

  // ⚠️ builder.sign() 반환값 시맨틱은 README에 명시 안 됨 — 런타임 검증 필요.
  const result = await builder.sign(signer, inputAsset, outputAsset);

  // 안전 분기: 어느 쪽이든 signed buffer 회수
  const signedBuffer = (outputAsset.buffer && outputAsset.buffer.length > 0)
    ? outputAsset.buffer
    : (result as unknown as Buffer);

  return { buffer: signedBuffer };
}
```

### A.5.1 Next.js 설정 (필수)

`apps/web/next.config.js`에 추가:
...
**이유**: 네이티브 `.node` 애드온이라 webpack 번들링에서 제외해야 정상 로드. 누락 시 빌드 통과해도 런타임 import 실패.

### A.5.2 라우트 런타임 (필수)

C2PA 호출 들어가는 모든 API Route에 명시:

```ts
export const runtime = 'nodejs';  // Edge 절대 안 됨 — 네이티브 애드온
```
...
| 단계 | 시간 |
|---|---|
| **A.10.0 Vercel 호환성 PoC** (스텁 import 라우트로 #58 검증) | 0.5일 |
| `@contentauth/c2pa-node` 학습 + 자체서명 인증서로 로컬 첫 매니페스트 생성 | 2~3일 |
| 코드 작성 (티어 분기 포함, builder.sign() 반환 시맨틱 확정) | 4~5일 |
| 테스트 + Preview 검증 | 2~3일 |
| **합계** | **약 2~2.5주** |

PoC 단계 실패 시 폴백 전환 — 그 경우 일정 +1주.
...
| **3** | 트랙 A C2PA `@contentauth/c2pa-node` 학습 + Vercel 호환성 PoC | ⭐⭐ | 1주 |
...
- @contentauth/c2pa-node (npm): https://www.npmjs.com/package/@contentauth/c2pa-node
- c2pa-node-v2 (GitHub repo): https://github.com/contentauth/c2pa-node-v2
- C2PA assertions/actions writing guide: https://opensource.contentauthenticity.org/docs/manifest/writing/assertions-actions/
