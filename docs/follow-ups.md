...
| A-63 | **C2PA 신뢰 목록 스냅샷 갱신** — 서버(c2pa-trust-list.ts)·c2patool 설정(docs/c2pa-trust-settings.json) 스냅샷이 2026-05-30자. `scripts/update-c2pa-trust-list.mjs` 재실행으로 갱신(신규 CA·TSA 반영). 9/2 실측에서 timeStamp.untrusted informational(판정 무영향)의 해소 후보. 주기: 분기 1회 권장 | 다음 유지보수 사이클 | P3 |
