...
| A-48 | **셔터 시점 해시 봉인 (verified 공격면 축소)** — 촬영(전처리)~[인증하기](후처리) 사이에 루팅 기기의 큐 파일 교체로 편집본이 verified 인증될 수 있는 갭(2026-08-26 대표 질의로 식별). 셔터 직후 백그라운드로 픽셀 해시 계산 → **Keychain/Keystore 키로 봉인 저장**(평문 큐 JSON 저장은 무효 — 파일과 함께 교체됨) → 인증하기 시점 재계산 대조, 불일치 시 거부. 효과: 가장 쉬운 공격(파일 편집)이 런타임 후킹 수준으로 상향. 제약: 워터마크 옵션 셔터 시점 확정(또는 유/무 2종 해시), 스탬핑 자체는 서버 final_hash 필요라 인증 시점 유지. 상세 분석: [verified-trust-model.md](verified-trust-model.md) §4-1 | 오픈 후 | P3 |
...
| 2026-08-26 | **verified 신뢰 모델 문서화 + A-48 등재** — 모바일 보안 재점검(8/25 — 알고리즘 공개 안전·SecureStore·번들 비밀 없음 확인, A-47 등재) 후속으로 대표 질의 심화: ①인증 전 큐 원본의 노출 평가(샌드박스 한정, Android 백업 규칙상 사진 미포함 — 서비스 무결성 영향 없음) ②**촬영~인증 갭 공격 식별**: 루팅 기기에서 큐 파일 교체로 편집본 verified 인증 가능(후킹 불요) ③대응책 "셔터 해시 봉인" 검토 — 하드웨어 키 봉인 필수·워터마크 옵션 선확정 제약 포함 부분 채택 판정 → **A-48**. 보증 범위·공격 비용 표·문구 가이드(금지 표현 포함)를 [verified-trust-model.md](verified-trust-model.md)로 신설 |
