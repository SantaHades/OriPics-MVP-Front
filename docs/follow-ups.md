...
| A-56 | **촬영 세부정보 수집·서명 확대 (Verified 상세 2단계)** — 1단계(2026-08-29 배포: PDF·판독·뷰어에 tier+C2PA 어서션의 플랫폼·렌즈·배율 표시)의 후속. 앱이 촬영 순간 **기기 모델명**(expo-device)·**ISO/셔터속도/조리개/초점거리**(VisionCamera photo metadata)를 읽어 /api/sign 메타에 포함 → 서버 서명+C2PA 어서션 기재 → 증명서·뷰어에 "편집 불가능한 촬영 세부정보"로 표시. 업로드본은 재인코딩 PNG라 EXIF가 없으므로(의도) 이 경로가 유일·상위 대안. A-49(무결성 수준 STRONG/DEVICE 기록)와 같은 빌드에 묶으면 효율적 — **빌드 8 포함 예정** (2026-08-29 대표 확정). 신규 인증부터 적용 | 빌드 8 | P2 |
