...
| A-50 | **모바일 소셜 로그인 (M1.5 실행)** — 앱 로그인 화면은 이메일 방식뿐이라 **웹 소셜(구글/카카오/네이버) 가입자는 password=null → 앱 로그인 불가**(2026-08-26 식별. 임시 우회=비밀번호 재설정으로 비번 생성 — 온보딩 안내문에 반영됨). 서버는 완비: `/api/mobile/auth/oauth`(google=id_token aud 검증·kakao/naver=access_token 프로필, 계정 연결 규칙·가입 보너스 동일) + `AuthContext.signInWithProviderToken` 준비됨. 잔여=클라 버튼+네이티브 인증 흐름+콘솔 등록: ①구글(네이티브 클라이언트 ID 등록→`GOOGLE_MOBILE_CLIENT_IDS` env) ②카카오 ③**Apple 로그인 — iOS는 서드파티 소셜 제공 시 App Store 4.8로 의무**(서버에 Apple identity token 검증+계정 연결 추가 필요) ④네이버(U-38 검수 통과 후). 클라 1~2일+콘솔 등록(대표 작업) | 정식 오픈 전 | P2 |
