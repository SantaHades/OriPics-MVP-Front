...
| A-50 | **모바일 소셜 로그인 (M1.5 실행)** — 앱 로그인 화면은 이메일 방식뿐이라 **웹 소셜(구글/카카오/네이버) 가입자는 password=null → 앱 로그인 불가**(2026-08-26 식별. 임시 우회=비밀번호 재설정으로 비번 생성 — 온보딩 안내문에 반영됨). 서버는 완비: `/api/mobile/auth/oauth` + `AuthContext.signInWithProviderToken`. **✅ Apple 트랙 완료(2026-08-26, `57e3670`)**: 콘솔(Services ID `com.santahades.oripics.web`·Key `27FA93NZQ8`·p8=`.secrets/apple/`) → 웹 NextAuth AppleProvider(client_secret 런타임 생성 `appleClientSecret.ts`, exp 180일 — **~2027-02 만료 전 재기동/재배포로 자동 갱신되나 키 회전 시 env 교체**) + pkce 쿠키 SameSite=None(form_post) + 로그인/가입 버튼 → 서버 apple 검증(JWKS, aud=번들ID) → 앱 iOS 공식 버튼(expo-apple-authentication, 심사 4.8 대응). Vercel env 4종(`APPLE_CLIENT_ID/TEAM_ID/KEY_ID/PRIVATE_KEY`) 설정 완료. 알려진 한계: Private Relay 이메일이면 기존 소셜 계정과 미매칭(신규 생성). ⏳**관찰 중(8/26 저녁)**: 웹 Apple **재로그인**(grant 재사용) 시 Apple 동의 화면에 "오류로 인해 요청을 완료할 수 없습니다" 배너 — 최초 로그인은 완전 정상, 재로그인도 [계속]/일반 로그인 폴백으로 완료 가능(기능 차단 아님). 배제됨: authorize 파라미터(실측 완벽)·콘솔 설정(도메인·Return URL 확인)·PKCE↔state(양쪽 동일)·Apple 장애(정상). 결론=당일 생성·수차례 재저장된 Services ID의 Apple측 전파 지연 추정 — **8/27 재확인**, 지속 시 scope 실험. 검증 방식은 state로 전환 유지(`checks:["state"]`+쿠키 SameSite=None — Apple 공식 지원 방식) **잔여**: ①구글(네이티브 클라이언트 ID 등록→`GOOGLE_MOBILE_CLIENT_IDS` env) ②카카오 ③네이버(U-38 검수 후). 옵션(수요 확인 시): Android용 Apple 로그인 — 네이티브 모듈이 iOS 전용이라 웹 OAuth 흐름(expo-auth-session) 별도 구현 필요(~반나절), 국내 Android의 Apple ID 보유율 낮아 보류(웹에서는 이미 전 플랫폼 동작) | 정식 오픈 전 (Apple 완료) | P2 |
