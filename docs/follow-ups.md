...
| 2026-08-26 | **verified 신뢰 모델 문서화 + A-48 등재·당일 구현** — 모바일 보안 재점검(8/25 — 알고리즘 공개 안전·SecureStore·번들 비밀 없음 확인, A-47 등재) 후속으로 대표 질의 심화: ①인증 전 큐 원본의 노출 평가(샌드박스 한정, Android 백업 규칙상 사진 미포함 — 서비스 무결성 영향 없음) ②**촬영~인증 갭 공격 식별**: 루팅 기기에서 큐 파일 교체로 편집본 verified 인증 가능(후킹 불요) ③대응책 "셔터 해시 봉인" 검토 — 하드웨어 키 봉인 필수·워터마크 옵션 선확정 제약 포함 부분 채택 판정 → **A-48 등재 후 당일 구현 완료**(`lib/capture/seal.ts` + 인증마크 pill 촬영 화면 이동 + 목록탭 칩 잠금, 실기기 검증 잔여). 보증 범위·공격 비용 표·문구 가이드(금지 표현 포함)를 [verified-trust-model.md](verified-trust-model.md)로 신설 |
