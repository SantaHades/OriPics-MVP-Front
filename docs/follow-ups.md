...
| U-34 | **모바일 attest 운영 설정 (M4 후속)** — 서버측 env·인프라·엔타이틀먼트 완료(2026-08-23, O-2 행 참조). **iOS 실기기 verified e2e 성공(2026-08-23, Tim's iPhone 로컬 빌드)**: production aaguid attestation 왕복 통과, DB `verified_proof` 기록 확인(`P260823-023701-344498`). 과정에서 서버 버그 2건 발견·수정: ①AAGUID_PROD 12→16바이트(production attestation 전면 거부 블로커) ②attest·receipt 거부 401→403(모바일 세션 파괴). 참고: 개발 서명 빌드도 엔타이틀먼트 production이면 production aaguid 발급됨(ALLOW_DEV 불필요, 설정했다 제거함). **Android 실기기 e2e 성공(2026-08-23, Galaxy A16 SM-A165N)**: Play Integrity 왕복 통과, DB `verified_proof` 기록(`P260823-054841-0044450`) — 단 사이드로드 빌드라 `GOOGLE_PLAY_INTEGRITY_ALLOW_UNRECOGNIZED=true` 임시 설정 하 검증(확인 후 즉시 제거·재배포). **TestFlight 릴리즈 빌드 verified 검증 완료(2026-08-23 15:17, 임시 플래그 전무·정식 경로)** — iOS는 완전 종결. **잔여: Android만 Play 내부 테스트 트랙 배포 시 PLAY_RECOGNIZED 포함 완전 검증(자동).** 검증용 임시 조치(tier flip·ALLOW_* env) 전부 원복 확인 | 모바일 베타(M7) 전 | — | P2 |
