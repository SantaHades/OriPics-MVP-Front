...
| 구매/등록 자격 | 로그인만 하면 가능 (free 포함). ~~구매자≠등록자 가능(선물)~~ → **⚠️2026-09-01 A안 변형(대표 확정)**: 포트원 회신(상품권 분류 리스크)에 따라 **판매분은 구매 계정 전용(양도 불가)** — redeem이 purchaser 일치 검증(payment_id 有 시), 선물 프레이밍 전면 제거. 시작 코드는 본인 활성화 토큰(웹 구매→앱 시작 경로용). **어드민 발급분(payment_id NULL, 베타 테스터 배포)은 종전대로 자유 등록**. 선물 기능 재도입은 상품권 심사 트랙 통과 후 재검토 |
| Verified | Pro와 동일하게 포함 — 패스 활성 중 free여도 verified 촬영 허용. **단 Verified는 기기 검증(attest) 통과 시에만 성사** — 미통과는 Standard 폴백+목록 고지, 무조건 부여안은 신뢰 모델 훼손으로 기각(8/31 대표 ① 확정). attest 실패는 1회 자동 재시도 |
