...
export function selectEmbedModeV3(width: number, height: number): EmbedMode {
  const borderCapacity = 2 * (width + height) - 4;
  if (borderCapacity >= PAYLOAD_BITS_V3) return 'b_only';
  if (borderCapacity * 3 >= PAYLOAD_BITS_V3) return 'rgb_lsb';
...
export async function computeBorderHashV3(
  ...
  if (mode === 'b_only') {
    for (let i = 0; i < PAYLOAD_BITS_V3; i++) {
      buf[8 + i * 4 + 2] &= 0xfe;
    }
  } else {
    const usedPixels = Math.ceil(PAYLOAD_BITS_V3 / 3);
...
export function embedPayloadV3(
  pixels: Uint8ClampedArray,
  width: number,
  height: number,
  payload: Uint8Array,
  ...
  if (payload.length !== PAYLOAD_LENGTH_V3) {
    throw new Error(`payload must be ${PAYLOAD_LENGTH_V3} bytes, got ${payload.length}`);
  }
  const coords = getBorderCoordinates(width, height);

  for (let bitIdx = 0; bitIdx < PAYLOAD_BITS_V3; bitIdx++) {
...
export function extractPayloadV3(
  pixels: Uint8ClampedArray,
  width: number,
  height: number,
  mode: EmbedMode,
  ...
  const coords = getBorderCoordinates(width, height);
  const out = new Uint8Array(PAYLOAD_LENGTH_V3);
...
export function splitPayloadV3(payload: Uint8Array): { meta: Uint8Array; finalHash: Uint8Array } {
  if (payload.length !== PAYLOAD_LENGTH_V3) {
    throw new Error(`payload length: ${payload.length}`);
  }
  return {
    meta: payload.subarray(0, META_LENGTH_V3),
    finalHash: payload.subarray(OFFSET_FINAL_HASH_V3, OFFSET_FINAL_HASH_V3 + HASH_LENGTH),
  };
}

export function buildPayloadV3(meta: Uint8Array, finalHash: Uint8Array): Uint8Array {
  if (meta.length !== META_LENGTH_V3) throw new Error('meta length');
  if (finalHash.length !== HASH_LENGTH) throw new Error('final_hash length');
  const out = new Uint8Array(PAYLOAD_LENGTH_V3);
  out.set(meta, 0);
  out.set(finalHash, OFFSET_FINAL_HASH_V3);
  return out;
}

export function readGpsFromMeta(meta: Uint8Array): { lat: number; lng: number } {
