...
import { downloadPdf, fmtBytes, fmtCaptured, fmtDateTime, getJson, type WMember, type WPhoto } from "@/lib/mailboxes/webClient";
...
export default function BackupDetailPage() {
  ...
  useEffect(() => {
    getJson<{ backup: BackupDetail }>(`/api/mailboxes/backups/${backupId}`).then((d) => setB(d.backup)).catch((e) => setErr((e as Error).message));
  }, [backupId]);

  const pdf = async () => {
    setBusy(true);
    try { await downloadPdf(`/api/mailboxes/backups/${backupId}/report?locale=${lang}`, `mailbox-backup-${backupId}.pdf`); }
    catch { alert(ko ? "PDF 생성에 실패했습니다." : "PDF generation failed."); } finally { setBusy(false); }
  };
  const remove = async () => {
    if (!b) return;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {err ? <p className="text-sm text-red-600">{ko ? "백업본을 열 수 없습니다: " : "Cannot open backup: "}{err}</p> : !b ? <RefreshCw className="animate-spin text-blue-500" /> : (
          <>
