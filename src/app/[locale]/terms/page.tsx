      ...
      <ol>
        <li>회사는 다음의 유료 플랜을 제공합니다. 모든 가격은 부가가치세(VAT) 포함 표기이며, 사업자 회원이 세금계산서를 요청하는 경우 별도 처리합니다.
          <ul>
            ...
            <li><strong>원데이 패스(1일 이용 상품)</strong>: 1장 ₩3,300 (단건 결제). 결제 시 패스가 구매 계정에 귀속되며(시작 코드 발급), 사용 시작(코드 등록) 시점부터 24시간 동안 모바일 촬영 인증 10회(사진 크기 무관)와 해당 인증 건의 공개링크·인증서 PDF 발급이 제공됩니다. 패스로 발행된 공개링크는 발행 시점부터 1년간 보관됩니다. 구매한 패스는 구매 계정 전용으로 타인에게 양도·재판매할 수 없고, 현금 환급·타 서비스 사용이 불가하며 선불전자지급수단에 해당하지 않습니다. 사용을 시작하지 않은 패스의 유효기간은 구매일로부터 1년이고, 계정당 동시에 1장의 패스만 활성화할 수 있습니다. 패스가 활성인 동안 촬영 인증은 플랜 제공 횟수에 우선하여 패스에서 차감됩니다.</li>
          </ul>
        ...
        <li><strong>할인권·이용권(파트너 릴레이 챌린지 등 프로모션)</strong>: 회사는 프로모션으로 Pro 플랜 월 요금에 적용되는 할인권(예: 50% 할인권) 또는 무료 이용권을 무상으로 발행할 수 있습니다. 할인권·이용권은 Pro 월간 구독 결제에만 자동 적용되며(첫 결제 최대 2장, 이후 결제마다 1장, 할인권 소진 후 무료 이용권 순), 원데이 패스·연간 결제·Business 플랜에는 적용되지 않습니다. 발급일로부터 24개월 유효하고, 현금 환급·양도·재판매가 불가하며 선불전자지급수단에 해당하지 않습니다. 할인권으로 결제액이 0원이 된 달에도 다음 달 자동 결제를 위해 결제 수단(카드) 등록이 필요합니다. 혜택이 적용된 결제의 환불액은 실제 결제한 금액을 기준으로 제11조에 따라 산정하며, 7일 이내 미사용 전액 환불(청약철회) 시에는 사용된 할인권을 복원하고 그 외의 부분 환불에서는 복원하지 않습니다. 허위 계정·자기 추천 등 부정한 방법으로 취득한 혜택은 사전 통지 후 회수될 수 있으며(이미 사용된 혜택은 회수하지 않음), 프로모션 참여 조건·기간·선착순 인원 등 세부 사항은 서비스 내 안내 페이지에 게시합니다.</li>
      </ol>
